...
setup = Setup(args.ll, args.qq, args.snr, 'capacity_predictors')
print("\n Running setup: " + setup.get_description(' ', '=') + "\n")

RIS_configs_train, avg_squared_mag_response_train, RIS_configs_test, avg_squared_mag_response_test = load_data(setup,
                                                                                                               together=False)
...
train_capacities = np.empty(avg_squared_mag_response_train.shape[0])
test_capacities = np.empty(avg_squared_mag_response_test.shape[0])
for i in range(avg_squared_mag_response_train.shape[0]): train_capacities[i] = compute_capacity(avg_squared_mag_response_train[i, :], setup.rho, setup.sigma_sq)
for i in range(avg_squared_mag_response_test.shape[0]): test_capacities[i] = compute_capacity(avg_squared_mag_response_test[i, :], setup.rho, setup.sigma_sq)

X_train = RIS_configs_train.astype(np.float32)
X_test = RIS_configs_test.astype(np.float32)

y_train = train_capacities
y_test = test_capacities
...
x = keras.layers.Dense(32, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(reg))(inp)
x = keras.layers.Dropout(p)(x)
x = keras.layers.Dense(32, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(reg))(x)
x = keras.layers.Dropout(p)(x)
...
out = keras.layers.Dense(output_neurons, activation='linear', name='out')(x)
...
capacities_rand_pred       = model.predict(random_RIS_profiles).flatten()
random_continuous_RIS      = np.random.uniform(0, 1, size=(1000, X_train.shape[1]))
capacities_continuous_rand = model.predict(random_continuous_RIS).flatten()
capacities_pred_train      = model.predict(X_train).flatten()
capacities_pred_test       = model.predict(X_test).flatten()
capacities_data_pred       = np.concatenate([capacities_pred_train, capacities_pred_test])

print()
print(
    f"data ground truth capacities                : mean {capacities_ground_truth.mean()}, std: {capacities_ground_truth.std()}")
print(
    f"data predicted capacities                   : mean {capacities_data_pred.mean()}, std: {capacities_data_pred.std()}")
print(
    f"random data predicted capacities            : mean {capacities_rand_pred.mean()}, std: {capacities_rand_pred.std()}")
print(
    f"random data (continuous)predicted capacities: mean {capacities_continuous_rand.mean()}, std: {capacities_continuous_rand.std()}")
